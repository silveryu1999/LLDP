...
def get_longest_dis_modified(nodes, dist_vec):
    dist = 0
    node_name = ''
    for name in nodes:
        if dist_vec[name][0] > dist:
...
def grouping(workflow: component.workflow, node_info):
    ...
    while True:
        ...
        check_nodes = workflow.nodes.copy()

        while Find_Flag == False:

            crit_length, tmp_node_name = get_longest_dis_modified(check_nodes, dist_vec)
            ...
            while tmp_node_name not in workflow.start_functions:
                crit_vec_temp[tmp_node_name] = prev_vec[tmp_node_name]
                tmp_node_name = prev_vec[tmp_node_name][0]

            if len(crit_vec_temp) == 0:
                all_in_same_set = False
            else:
                all_in_same_set = True
                for k, v in crit_vec_temp.items():
                    set1 = find_set(k, group_set)
                    if v[0] not in set1:
                        all_in_same_set = False
                        break
