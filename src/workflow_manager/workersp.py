...
class WorkerSPManager:
    def __init__(self, host_addr: str, workflow_name: str, data_mode: str, function_info_addr: str):
        ...
        # store func ip and to info per request here, remember to delete after execution
        self.ip_to_lock = gevent.lock.BoundedSemaphore() # guard self.func_ip_to_per_request_dict
        self.func_ip_to_per_request_dict: Dict[str, dict] = {}
    ...
    def del_state(self, request_id: str, master: bool):
        ...
        # clear ip and to info
        self.ip_to_lock.acquire()
        if request_id in self.func_ip_to_per_request_dict:
            del self.func_ip_to_per_request_dict[request_id]
        self.ip_to_lock.release()
    ...
    # get function info per request
    # 'ip' and 'to' is dynamic
    # return the same format as get_function_info(), and additional ip and to info
    def get_function_info_per_request(self, function_name: str, request_id: str) -> Any:
        # first get origin static function info
        origin_info = self.get_function_info(function_name)
        # then get ip and to info from redis or memory object
        self.ip_to_lock.acquire()
        if request_id not in self.func_ip_to_per_request_dict:
            self.func_ip_to_per_request_dict[request_id] = repo.get_function_ip_to(request_id)
        ip_to_info = self.func_ip_to_per_request_dict[request_id][function_name]
        self.ip_to_lock.release()

        return origin_info, ip_to_info
    ...
    def trigger_function(self, state: WorkflowState, function_name: str, no_parent_execution = False) -> None:
        ...
        _, ip_to_info = self.get_function_info_per_request(function_name, state.request_id)

        if ip_to_info['ip'] == self.host_addr:
            # function runs on local
            self.trigger_function_local(state, function_name, no_parent_execution)
        else:
            # function runs on remote machine
            self.trigger_function_remote(state, function_name, ip_to_info['ip'], no_parent_execution)
    ...
    def run_function(self, state: WorkflowState, function_name: str) -> None:
        ...
        func_info, ip_to_info = self.get_function_info_per_request(function_name, state.request_id)

        # switch functions
        if function_name.startswith('virtual'):
            self.run_switch(state, func_info)
            return # do not need to check next
        
        if function_name in self.foreach_func:
            self.run_foreach(state, func_info, ip_to_info)
        elif function_name in self.merge_func:
            self.run_merge(state, func_info, ip_to_info)
        else: # normal functions
            self.run_normal(state, func_info, ip_to_info)
    ...
    def run_foreach(self, state: WorkflowState, info: Any, ip_to_info: Any) -> None:
        start = time.time()
    ...
    def run_merge(self, state: WorkflowState, info: Any, ip_to_info: Any) -> None:
        start = time.time()
        all_keys = repo.get_keys(state.request_id)  # {'split_keys': ['1', '2', '3'], 'split_keys_2': ...}
        self.function_manager.run(info['function_name'], state.request_id,
                             info['runtime'], info['input'], info['output'],
                             ip_to_info['to'], all_keys)
        end = time.time()
        repo.save_latency({'request_id': state.request_id, 'function_name': info['function_name'], 'phase': 'all', 'time': end - start})

    def run_normal(self, state: WorkflowState, info: Any, ip_to_info: Any) -> None:
        start = time.time()
        self.function_manager.run(info['function_name'], state.request_id,
                             info['runtime'], info['input'], info['output'],
                             ip_to_info['to'], {})
