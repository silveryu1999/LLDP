...
def run_workflow(workflow_name, request_id, spawn_rate, parse_mode):
    ...
    try:
        rep = requests.post(url, json=data, timeout=1000)
...
def run_workflow_closed(workflow_name, request_id, parse_mode):
    print('----firing workflow----', request_id)
    url = 'http://' + config.GATEWAY_ADDR + '/run'
    data = {'workflow': workflow_name, 'request_id': request_id, 'parse_mode': parse_mode}
    try:
...
def analyze(datamode):
    ...
    for workflow in workflow_pool:
        ...
        print(f'----skipping prewarming, start from cold----')
        ...
        while ticket > 0:
            res = run_workflow_closed(workflow, str(uuid.uuid4()), parse_mode)
            if res == False:
                break
            ticket -= 1
            gevent.sleep(1)
    ...
    df2 = pd.read_csv(parse_mode + '_closed.csv')
    # df2 = pd.DataFrame()
    for workflow in workflow_pool:
        col = workflow + '_' + str(nodes)
        if col in df2.columns:
            df2[col] = pd.Series(all_latencies[workflow])
        else:
            df2.insert(df2.shape[1], col, pd.Series(all_latencies[workflow]))
    df2.to_csv(parse_mode + '_closed.csv')
